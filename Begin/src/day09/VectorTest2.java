...
public class VectorTest2 {
	...
	public static void main(String[] args) {
		// [1] Person 객체를 3개 생성하세요
		Person p1 = new Person("홍길동", 22);
		Person p2 = new Person("김강찬", 46);
		Person p3 = new Person("최재우", 85);

		// [2] Persion을 저장할 Vector를 생성해서 3개 객체를 저장하세요
		Vector<Person> v = new Vector<>();
		v.add(p1);
		v.add(p2);
		v.add(p3);

		// [3] for루프를 이용해서 벡터에 저장된 Person의 이름과 나이를 출력하세요

		for (Person s : v) {
			System.out.println(s.getName() + " 나이 : " + s.getAge() + "세");
		}

		/* Enumeration<E> elements(): 벡터에 저장된 객체들을 한꺼번에 꺼내오고자 할 때 사용.
		...
		// Q. Iterator<E> iterator() 메서드 활용해서 v의 요소들 이름#나이 출력하기
		...
		// void removeElements : 벡터에 저장된 객체 모두 지우기
		
		System.out.println("v.size: "+v.size());
		v.clear();
		System.err.println("--- clear 이후 ---");
		System.out.println("v.size: "+v.size());
